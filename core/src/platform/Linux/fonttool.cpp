...
int main(int argc, const char *argv[]) {
  ...
  if (cmdl[1] == "buildCache") {
    ...
    ASSERT(tp);
    const std::vectoelse if (cmdl[1] == "outline") {
    std::string text = cmdl[2];
    std::string fontName = cmdl[3];

    TypefaceLoader *tl = TypefaceLoader::getInstance();
    ASSERT(tl);
    if (!tl->importFelse if (cmdl[1] == "outline") {
    std::string text = cmdl[2];
    std::string fontName = cmdl[3];
    ...
    return 1;
      ...
      INFO("Please uelse if (cmdl[1] == "outline") {
    std::string text = cmdl[2];
    std::string fontName = cmdl[3];
    ...
    usage(cmdl[0]);
    return 1;
  }Cache` to build font cache first.");
      return 1;
    }
    tl->dumpFontCachelse if (cmdl[1] == "outline") {
    std::string text = cmdl[2];
    std::string fontName = cmdl[3];
    ...
    usage(cmdl[0]);
    return 1;
  }ceProvider::Typeface *> &faces = tp->selectTypefaces(fontName);
